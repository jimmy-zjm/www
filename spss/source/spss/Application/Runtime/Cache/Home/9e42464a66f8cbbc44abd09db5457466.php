...
//		导航购物车的左切换
 ...
 <link rel="stylesheet" href="/spss/source/spss/Public/css/public.css" />
<link rel="stylesheet" type="text/css" href="/spss/source/spss/Public/css/producelist.css"/>
<div class="top"><img src="<?php echo ($image); ?>" alt="" /></div>
<div class="main">
	<div class="content">

		<div class="cont1">
			<p><input type="text" id='key' name='keyword' onkeydown="keydown()" value="<?= $_GET['keyword'] ?>"><span onclick='key()'></span></p>
		</div>
		...
		<?php foreach ($list as $k => $v): ?>
			<div class="cont2">
				...
				<?php foreach ($v as $val): ?>
					<span style="<?= $val['id']==$_GET['id']?'color:red':'color:#000' ?>" class="changered" value="<?php echo ($val["id"]); ?>"><?php echo ($val["name"]); ?></span>
				<?php endforeach ?>
			</div>
		<?php endforeach ?>
		<div class="cont4">
			<?php foreach ($data as $k => $v): ?>
				<li style="margin-right: 18px"> 
					<a href="<?php echo U('info',array('id'=>$v['id']));?>">
...
<script>
	$(".changered").click(function(){
		href('id',$(this).attr('value'));
	})

	function key(){
		href('keyword',$('#key').val());
	}

	function href(k,v){
		location.href = "/spss/source/spss/index.php/Material/floorList/"+k+"/"+v;
	}
