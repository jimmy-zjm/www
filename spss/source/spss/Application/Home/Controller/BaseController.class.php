...
class BaseController extends Controller {
    ...
    public function cartData(){
    	...
    	$data  = $model->getCartByUserId($_SESSION['user']['userId']);
    	$cartPrice = 0;
    	$cartNum   = 0;
    	foreach ($data as $k => $v) {
    		if($v['class']==9){
    			if($v['num']>1){
    				$cartPrice += ($v['price']+$v['price']*($v['num']-1)*0.9);
    			}else{
    				$cartPrice += $v['price']*$v['num'];
    			}
    		}else{
	    		$cartPrice += $v['price']*$v['num'];
	    	}
    ...
    //获取跳转URL地址
    public function url(){
        // if (ACTION_NAME!='doLogin' && ACTION_NAME!='register' && ACTION_NAME!='doRegister'  && ACTION_NAME!='changePass' && ACTION_NAME!='login' && !isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    public function doLogin(){
		...
		if(!preg_match("/^1[34578]\d{9}$/", $mobile_phone))  $this->error('请输入正确手机号码');
		if (!empty($_POST)){
