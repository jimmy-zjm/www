...
class EuOrderModel extends Model {
	protected $trueTableName = 'xgj_eu_order';
    ...
    public function placeOrder(){
        ...
        if($order_id !== false){
            ...
            //分单
            $orderInfo = M("xgj_eu_order_goods")->where(array('order_id'=>$order_id,'user_id'=>$user_id))->select();

            $info_['order_id'] = $order_id;
            $info_['user_id']  = $user_id;
            foreach ($orderInfo as $key => $v) {
                $info_['split_sn']     = $data['sn'].'-'.$v['id'];
                $info_['detail_id']     = $v['id'];
    ...
    public function alipay($order_id,$class_id){
        //查询出订单总价
        $order        = M('xgj_eu_order')->where(array('id'=>$order_id))->find();
    ...
    public function chinaPayOrder($order_id,$class_id){
        ...
        include("__ROOT__/Chinapay/util/common.php");
        //查询出订单信息
        $order       = M('xgj_eu_order')->where(array('id'=>$order_id))->find();        
        //查询出订单号
        $order_code  = $order['sn'];
    ...
    public function noPayOrder($order_id,$class_id){
        ...
        header('Content-Type:text/html;charset=utf-8 ');
        ini_set('date.timezone','Asia/Shanghai');
        include("__ROOT__/Nopay/util/common.php");
        //查询出订单信息
        $order       = M('xgj_eu_order')->where(array('id'=>$order_id))->find();        
        //查询出订单号
        $order_code  = $order['sn'];
