...
class TypeController extends AdminController{
    ...
    public function __construct(){
        parent::__construct();
        $this->m = new \Admin\Model\Overseas\TypeModel;
        $this->class_id = 2;
    }
    ...
    public function delete($id){
        ...
        if($this->m->where('id='.$id)->delete()){
            //删除成功
            //删除相应的属性
            M('xgj_ov_attribute')->where(array('type_id'=>$id))->delete();
    ...
    public function toggle(){
        $id = I('get.id/d');
        if(M()->execute("UPDATE xgj_ov_type SET is_use = is_use^1 WHERE id=$id")){
            echo 1;
        }else{
            echo -1;
        }
