...
class AttributeController extends AdminController{
    ...
    public function __construct(){
        parent::__construct();
        $this->m = new \Admin\Model\Overseas\AttributeModel;
    }
    ...
    */
    public function index($id){
        ...
        //取出商品类型的数据
        $type = M('xgj_ov_type')->where(array('id'=>$type_id))->find();
        $type_list = M('xgj_ov_type')->where(array('is_use'=>1,'class_id'=>2))->select();
    ...
    /*显示添加属性的页面*/
    public function add(){
        $data = M('xgj_ov_type')->where(array('is_use'=>1,'class_id'=>2))->select();
    ...
    public function delete(){
        ...
        if(M('xgj_ov_attribute')->delete($id)){
            $this->success('删除成功',U('index', $args));
            die;
        }
        $this->error(M('xgj_ov_attribute')->getError());
    }
    ...
    */
    public function edit($id, $type_id){
        $id = (int)$id;
        $type_id = (int)$type_id;
        $data = M('xgj_ov_attribute')->where('id='.$id)->find();
