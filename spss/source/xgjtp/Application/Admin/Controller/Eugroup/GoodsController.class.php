...
class GoodsController extends AdminController{
    ...
    public function __construct(){
        parent::__construct();
        $this->m = new \Admin\Model\Eugroup\GoodsModel;
    }
    ...
    public function ajaxDelAttr(){
        $id = I('id');
        $re=M('xgj_eu_goods_attr')->where(array('id'=>$id))->delete();
        if($re){
            echo '1';
        }else{
            echo '删除错误';
    ...
    public function deleteImage($id,$type=''){
        ...
        if(empty($id)) die;
				$type=!empty($type)?$type:'0';
		if($type=='1')
			$table=M('xgj_eu_video');
		else 
			$table=M('xgj_eu_image');
        $image = $table->find($id);
        if($table->delete($id)){
            //删除图片文件
    ...
    public function getAttrListByTypeId($id){
        $id = (int)$id;
        $data = M('xgj_eu_attribute')->where(array('type_id'=>$id))->select();
        $html = '';
