...
class BrandController extends AdminController{
    ...
    public function index(){
        $model = new \Admin\Model\Eugroup\BrandModel;
        $data = $model->getAll();
    ...
    public function delete(){
        $id = I('get.id/d');
        $info = M('xgj_eu_brand')->where('id='.$id)->find();
        if(M('xgj_eu_brand')->delete($id)){
            //删除图片和缩略图
            deleteImage($info['logo'],C('IMG_THUMB_BRAND'));
            $this->success('删除成功',U('index?page='.I('get.page')));
            die;
        }
        $this->error(M('xgj_eu_brand')->getError());
    }

    /*显示编辑品牌的页面*/
    public function edit($id){
        $id = (int)$id;
        $data = M('xgj_eu_brand')->where('id='.$id)->find();
    ...
    public function toggle(){
        $id = I('get.id/d');
        if(M()->execute("UPDATE xgj_eu_brand SET is_show = is_show^1 WHERE id=$id")){
            echo 1;
        }else{
            echo -1;
        }
