...
class QuoteController extends AdminController{
    ...
    public function index(){
        ...
        // var_dump($data);exit;
        //分页  
        $data1 = M('xgj_furnish_quote')->select();

        $total = count($data1);

        $page  = getPage($total,C('MANAGER_PAGE_SIZE'));

        $data  = M('xgj_furnish_quote')->limit($page['limit'])->select();
    ...
    public function edit(){
        ...
        $data  = M('xgj_furnish_quote')->where("quote_id=$quote_id")->find();

        $cate = M('xgj_furnish_cat')->select();
       ...
       /*   $explains = M('xgj_quote_explain')->where($map)->order('number asc')->select();
      ...
      $explain  = M('xgj_quote_explain')->where($map)->order('number asc')->limit($page['limit'])->select();
    ...
    public function edit_save(){
        ...
        $puzzled            = I('post.puzzled');                      //困惑与需求
        $product            = I('post.product');                        //产品与品牌
        $_POST['starttime'] = strtotime(I('post.starttime'));           //上架时间
        $_POST['endtime']   = strtotime(I('post.endtime'));             //下架时间   

        if (empty($puzzled)) $this->error('请填写困惑与需求');
        if (empty($product)) $this->error('请填写产品与品牌');
        ...
        //查询当前系统的信息
        $quoteData = M('xgj_furnish_quote')->where("quote_id=$id")->find();   
        ...
        $data = D('xgj_furnish_quote')->create();

        $updata = M('xgj_furnish_quote')->where("quote_id=".$id)->save($data);
    ...
    public function add_save(){
        ...
        // $standard  = I('post.standard');                       //施工标准
        // $customer  = I('post.customer');                       //售后
        $puzzled      = I('post.puzzled');                      //详情介绍
        $product      = I('post.product');                        //满足需求
        ...
        $sel = M('xgj_furnish_quote')->field('quote_name')->select();
        ...
        if ($image['code']==1 && $video['code']==1 && $video_img['code']==1 &&  $manual['code']==1 && $product_img['code']==1) {    
            ...
            $add = M('xgj_furnish_quote')->add($data);
    ...
    public function delete(){
        ...
        $data = M('xgj_furnish_quote')->field('img,video,video_img')->where("quote_id=".$id)->select();
        ...
        $del = M('xgj_furnish_quote')->where("quote_id=$id")->delete();
    ...
    public function quote_list(){
        ...
        //获取系统名称
        $quote=M('xgj_furnish_quote')->where("quote_id=$quote_id")->select();
        //查询一条记录
        $child_list=M('xgj_quote_child_list ch,xgj_furnish_goods g')->where("ch.goods_sn=g.goods_sn and ch.quote_id=$quote_id and ch.level=$level")->order(array('sort'=>'asc'))->select();

        $quote_type = M('xgj_quote_type')->where("quote_id=$quote_id")->select();
    ...
    public function child_list_adds(){
        ...
        $quote = M('xgj_quote_child_list')->where("level=$level and chf_id=$chf_id and quote_id=$quote_id and goods_id=$goods_id")->select();
    ...
    public function child_list_add(){
        ...
        //添加一条记录
        $child_id=M('xgj_quote_child_list')->add($data);
        //清楚之前的输出
        ob_start();
        //获取一条信息    
        $item=M('xgj_quote_child_list ch,xgj_furnish_goods g')->where("ch.goods_id=g.goods_id and g.goods_id=$goods_id and ch.child_id=$child_id")->select();

        //输出一条信息内容
        echo "<tr id='child_list_tr_".$item['0']['child_id']."'>
        <td><input type='text' id='sort".$item['0']['child_id']."' class='quinput' style='width:40px;' value='".$item['0']['sort']."' onchange='sort(".$item['0']['child_id'].",this.value,".$item['0']['sort'].")'/></td>
        ...
        |   
        <a href=".U('quote_ini_edit',array('childId'=>$item['0']['child_id'])).">修改编码</a>
        |
        <input type='button' value='移除' onclick='remove_list(".$item['0']['child_id'].")'>
        |
        <a href=".U('rule',array('id'=>$item['0']['child_id'],'quote_id'=>$item['0']['quote_id'],'level'=>$item['0']['level'])).">编辑规则</a>";
        if (!empty($item['0']['formula'])) {
    ...
    public function child_list_del(){   
        ...
        $row = M('xgj_quote_child_list')->where("child_id=$child_id")->find();

        $list = M('xgj_quote_child_list')->where("quote_id={$row['quote_id']} and `level`={$row['level']}")->select();
        ...
        //删除一条记录
        $del=M('xgj_quote_child_list')->where("child_id=$child_id")->delete();
        //返回值
        if ($del == 1) {      
            echo 'success';
        }
    ...
    public function quote_ini_edit(){
        ...
        $list = M('xgj_quote_child_list l')->join('xgj_furnish_goods g on l.goods_sn = g.goods_sn')->where("l.child_id = $childId")->find();
    ...
    public function saveGoodsSn(){
        ...
        $list = M('xgj_quote_child_list')->where($map)->save($updata);
    ...
    public function do_quote_ini_edit(){
        ...
        $list = M('xgj_quote_child_list')->where("child_id=$child_id")->save($updata);
    ...
    public function fenlei(){
        $id = I('get.quote_id');
        $list = M('xgj_quote_type')->where("quote_id=$id")->select();
    ...
    public function delfenlei(){
        $id = I('get.id');
        $quote_id = I('get.quote_id');
        $del = M('xgj_quote_type')->where("id=$id")->delete();
        if ($del==1) {
            $list = M('xgj_quote_type')->where("quote_id=$quote_id")->select();
            foreach ($list as $key => $value) {
                echo '<li>
                    <label>类名</label>
                    <input id="dddd'.$key.'" name="formula" type="text" class="dfinput" value='.$value['text'].' required="required"/>
    ...
    public function savefenlei(){
        ...
        $text = I('get.text');
        $data['text'] = $text;
        $save = M('xgj_quote_type')->where("id=$id")->save($data);
        if ($save==1) {
            $list = M('xgj_quote_type')->where("quote_id=$quote_id")->select();
            foreach ($list as $key => $value) {
                echo '<li>
    ...
    public function optionfenlei(){
        ...
        $level = I('get.level');    
        $list = M('xgj_quote_type')->where("quote_id=$id")->select();
        $data = M('xgj_quote_child_list ch,xgj_furnish_goods g')->where("ch.goods_sn=g.goods_sn and ch.child_id=$cid")->select();
        // echo $data['0']['type'];exit;
        if (!empty($data['0']['type'])) {
            $type = M('xgj_quote_type')->where('id='.$data['0']['type'])->select();
    ...
    public function dooptionfenlei(){
        ...
        $updata = M('xgj_quote_child_list')->where("child_id=$cid")->save($data);
        if ($updata == 1) {
    ...
    public function rulechange(){
        ...
        $map['l.level'] = $lv;
        $map['l.goods_sn|g.goods_name'] = ['like','%'.$val.'%'];

        $list = M('xgj_quote_child_list l')->join('xgj_furnish_goods g on l.goods_sn = g.goods_sn')->join('xgj_furnish_quote q on q.quote_id = l.quote_id')->field("q.quote_name,l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where($map)->select();
        // var_dump($list);exit;
        ...
        // $list = M('xgj_quote_child_list l,xgj_furnish_goods g')->field("l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where($where)->select();
        if ($qid != 100) {
            ...
            foreach ($list as $key => $v) {
                echo '<option value="'.$v['id'].'">系统：'.$v['quote_name'].'，材料：'.$v['name'].'，型号：'.$v['model'].' ，编码：'.$v['sn'].'</option>';
            }
        }
    ...
    public function rule(){    
        ...
        $map['l.quote_id'] = $quote_id;
        $map['l.level'] = $level;
        $map['l.child_id'] = ['neq',$id];

        $list = M('xgj_quote_child_list l')->join('xgj_furnish_goods g on l.goods_sn = g.goods_sn')->join('xgj_furnish_quote q on q.quote_id = l.quote_id')->field("q.quote_name,l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where($map)->select();

        // $list = M('xgj_quote_child_list l,xgj_furnish_goods g')->field("l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where("l.quote_id = $quote_id and l.level = $level and l.goods_sn = g.goods_sn and l.child_id<>$id")->select();
        
        $row  = M('xgj_quote_child_list l,xgj_furnish_goods g')->field("l.child_id id,l.complex complex,g.goods_name name,g.goods_model model,g.goods_sn sn")->where("l.goods_sn = g.goods_sn and l.child_id=$id")->find();

        
        $data = M('xgj_quote_child_list')->where("child_id=$id")->select();

        if (!empty($row['complex'])) {
            $complex = explode('|', $row['complex']);
            $_SESSION['docomplex'][$id] = $complex;

            // $complex = implode(',', $complex);

            // $where['child_id'] = ['in',$complex];
            // $complexList = M('xgj_quote_child_list l')->join('xgj_furnish_goods g on l.goods_sn = g.goods_sn')->field("l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where($where)->select();
        ...
        if(is_numeric($data['0']['guanlian'])){
            $where['child_id'] = $data['0']['guanlian'];
            $re = M('xgj_quote_child_list l')->join('xgj_furnish_goods g on l.goods_sn = g.goods_sn')->join('xgj_furnish_quote q on q.quote_id = l.quote_id')->field("q.quote_name,l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where($where)->find();
            $this->assign('id',$re['id']);
            $this->assign('quote_name',$re['quote_name']);
            $this->assign('name',$re['name']);
            $this->assign('model',$re['model']);
            $this->assign('sn',$re['sn']);
        }
        ...
        if (!empty($data['0']['typenumgt'])) {
            $decides = explode('|', $data['0']['typenumgt']);
            $decide  = explode(',', $decides['1']);
            $this->assign('typenumgt',$decides['0']);
            $this->assign('decide',$decide);
        }

        $house_type = explode(',', $data['0']['house_type']);
    ...
    public function dorule(){
        ...
        $house_type  = I('post.house_type');  //房屋类型
        ...
        $housemax    = I('post.housemax');    //房屋面积小于等于
        $housemin    = I('post.housemin');    //房屋面积大于
        
        $peoplemax   = I('post.peoplemax');   //常住人口小于等于
        $peoplemin   = I('post.peoplemin');   //常住人口大于
        $usenum      = I('post.usenum');      //数量等于多少使用1个
        
        $usenum      = I('post.usenum');      //数量等于多少使用1个
        
        $decide      = I('post.decide');      //多项选中选中房间
        $typenumgt   = I('post.typenumgt');   //多项选中房间数量大于多少使用
        
        $append      = I('post.append');      //追加公式
        $gongshiceil = I('post.gongshiceil'); //公式按多少取整


        if (!empty($typenumgt)) {
            $decides = '';
            for ($i=1; $i <=5 ; $i++) { 
                if (empty($decide[$i])) $decides .= ',0';
                else $decides .= ','.$decide[$i];
            }
            $decide = trim($decides,',');
            $typenumgt .= '|'.$decide;
        }

        if (!empty($house_type)) $house_type = implode(',', $house_type);
        ...
        if (!empty($order)) {
            //当前材料编号
            $idSort = M('xgj_quote_child_list')->where("child_id=$id")->find();

            $returnSort = ture;
            //关联多个材料
            if ($order=='complex') {
                $where = ' child_id = '.implode(' or child_id = ', $_SESSION['docomplex'][$id]);
                $complexSort = M('xgj_quote_child_list')->field('sort')->where("$where")->select();
                foreach ($complexSort as $k => $v) {
                    if ($v['sort'] >= $idSort['sort'] ) $returnSort = false;
                }
            }else if(preg_match("/^[0-9]+$/", $order)){
                //关联单个材料
                $orderSort = M('xgj_quote_child_list')->where("child_id=$order")->find();
                if ($orderSort['sort'] >= $idSort['sort'] ) $returnSort = false;
            }

            if ($returnSort == false && $quote_id!=100) {
        ...
        if(!empty($order)){
            $selectRow = M('xgj_quote_child_list')->where("child_id = $id")->find();
        ...
        if (!empty($order) && $order == 'area') {
            ...
            $select = M('xgj_quote_child_list')->where("child_id = $order")->select();
        ...
        $formulas = array(
            ...
            'typenumgt'   =>$typenumgt,
            'append'      =>$append,
            'gongshiceil' =>$gongshiceil,
            );

        $save = M('xgj_quote_child_list')->where("child_id=$id")->save($formulas);
    ...
    function docomplex($o=null,$id=null,$i=null){
        ...
        foreach ($_SESSION['docomplex'][$id] as $k => $v) {
            $row = M('xgj_quote_child_list l,xgj_furnish_goods g')->field("l.child_id id,g.goods_name name,g.goods_model model,g.goods_sn sn")->where("l.goods_sn = g.goods_sn and l.child_id=$v")->find();
    ...
    function deleteimages(){
        ...
        @unlink($img);
        $data = M('xgj_furnish_quote')->field('images')->where("quote_id=".$_GET['id'])->find();
        ...
        $save = M('xgj_furnish_quote')->where("quote_id=".$_GET['id'])->save($image);
        echo $save==true?'1':'2';
    }
    ...
    function editBatch(){
        ...
        $save = M('xgj_quote_child_list')->where("child_id=$child_id")->save($batch);
        echo $save;
    }
    ...
    public function sort(){
        ...
        $sort    = M('xgj_quote_child_list')->where("child_id=$id")->find();
        $where   = "`quote_id` = '{$sort['quote_id']}' and `level` = '{$sort['level']}'";

        $os = M('xgj_quote_child_list')->where("$where and sort='{$data['sort']}'")->find();

        if (!empty($os)) {
            echo '修改失败！该编号已存在！';exit;
        }

        $sortAll = M('xgj_quote_child_list')->where($where)->select();
        ...
        foreach ($sortAll as $k => $v) {
            if ($v['guanlian'] == $id && $data['sort'] >= $v['sort']) {
                ...
                $complexAll = M('xgj_quote_child_list')->where($complex)->select();
        ...
        $update = M('xgj_quote_child_list')->where("child_id=$id")->save($data);
        echo $update;
    }
    ...
    public function judge(){
        ...
        $judge = M('xgj_quote_judge')->select();
        foreach ($judge as $k => $v) {
            if ($v['judge']==1) $nan[] = $v;
            if ($v['judge']==2) $bei[] = $v;
        }

        $area = M('xgj_area')->where(array('pid'=>100000))->field('id,name')->select();
    ...
    public function addJudge(){
        ...
        $isCity = M('xgj_quote_judge')->where("city='{$city}'")->find();
        if (!empty($isCity)) {
            echo 'error';exit;
        }

        $add = M('xgj_quote_judge')->add($data);
        $judge = M('xgj_quote_judge')->select();
        foreach ($judge as $k => $v) {
            if ($v['judge']==1) $nan[] = $v;
            if ($v['judge']==2) $bei[] = $v;
        ...
        foreach ($nan as $key => $value) {
            echo '<span id="judge1'.$value['id'].'" style="height:25px;"> 城市：'.$value['city'].'<a style="margin-left:30px;" href="javascript:;" onclick="del('.$value['id'].',1)">删除</a></span>';
        }
        echo '</div></label>';
        ...
        foreach ($bei as $key => $value) {
            echo '<span id="judge2'.$value['id'].'" style="height:25px;"> 城市：'.$value['city'].'<a style="margin-left:30px;" href="javascript:;" onclick="del('.$value['id'].',2)">删除</a></span>';
        }
        echo '</div></label>';
    }
    ...
    public function delJudge(){
        $id = I('get.id');
        $del = M('xgj_quote_judge')->where("id='{$id}'")->delete();
        echo $del;
    } 
    ...
    public function insertDemand(){
        ...
        $data = M('xgj_quote_explain')->create();
        $re = M('xgj_quote_explain')->add($data);
        if($re>0){
    ...
    public function demandEdit(){
        $id = I('get.id');
        $map['id'] = $id;
        $data = M('xgj_quote_explain')->where($map)->find();
    ...
    public function doDemandEdit(){
        ...
        if (!empty($_FILES['img']["name"])) {
            $oldImg = M('xgj_quote_explain')->where($map)->getField('img');
        ...
        $data = M('xgj_quote_explain')->create();
        unset($data['id']);
        $re = M('xgj_quote_explain')->where($map)->save($data);
        if($re!==false){
    ...
    public function demandDel(){
        ...
        $quote_id = I('post.quote_id');
        $oldImg = M('xgj_quote_explain')->where($map)->getField('img');
        $re = M('xgj_quote_explain')->where($map)->delete();
        if($re>0){
            unlink('./Public/Uploads/'.$oldImg);
    ...
    public function area(){
        $map['name'] = I('get.v');
        $map['pid']  = 100000;
        $id = M('xgj_area')->where($map)->getField('id');
        $re = M('xgj_area')->field('id,name')->where(array('pid'=>$id))->select();
        if (!empty($re)) echo json_encode($re);
    }
}