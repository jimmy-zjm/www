...
class BrandModel extends Model{
    //指定真实表名
    protected $trueTableName = 'xgj_ov_brand';

    //指定字段
    protected $fields = array('id','description','is_show', 'name', 'url', 'order','cate_id','class_id');
    protected $pk = 'id';
    ...
    public function cate_list($pid = 0, $num = 0, $curid = 0){
        ...
        $result=M('xgj_ov_category')->where("pid=$pid and class_id =2")->select();
        $str = str_repeat ( '+', $num );
        $num += 2;
        $string='';
        foreach ($result as $v){
