...
class GoodsModel extends Model{
    protected $trueTableName = 'xgj_eu_goods_new';
    protected $fields= array('id','goods_title','goods_title_style','pro_info','goods_sn','cate_id','brand_id','type_id','market_price','shop_price','groupbuy_price','face_image','is_putaway','is_new','is_hot','is_rec','seo_keywords','seo_description','description','weight','weight_unit','warn_number','goods_detail','seller_note','is_delete','click_count','addtime','package_list','after_sales_support','duties','luggage','purchase','vat','service_charge','discount_amount','integral','goods_mnemonic','goods_explain' );
    ...
    protected $_validate = array(
            ...
            array('integral','currency','积分额度不合理',0),
            array('shop_price','require','本店售价为必填',1),
            array('shop_price','currency','本店售价不合理',1),
            ...
            // array('particulars','require','商品详情为必填',1),

            // array('duties','require','关税税率为必填',1),
            // array('vat','require','增值税税率为必填',1),
            // array('service_charge','require','服务费税率为必填',1),
            // array('luggage','require','邮递价格为必填',1),
        );

    /*
    验证团购数量区间
     */
  /*  protected function checkGroupbuy(){
        if(I('post.is_groupbuy')){
            $groupbuy_min_num = I('post.groupbuy_min_num/d');
            $groupbuy_max_num = I('post.groupbuy_max_num/d');
            if($groupbuy_min_num< 1 || $groupbuy_max_num< 1 || ($groupbuy_min_num-$groupbuy_max_num)>0){
                return false;
            }
        }
        return true;
    ...
    验证团购日期
    */
  /*  protected function checkGroupbuyDate(){
        if(I('post.is_groupbuy')){
            $groupbuy_start_date = I('post.groupbuy_start_date');
            $groupbuy_end_date = I('post.groupbuy_end_date');
            if(empty($groupbuy_start_date) || empty($groupbuy_end_date) || (strtotime($groupbuy_start_date)-strtotime($groupbuy_end_date))>0){
                return false;
            }
        }
        return true;
    ...
    protected function _before_insert(&$data, $options){
        ...
        //$data['is_groupbuy']         = I('post.is_groupbuy')?1:0;
        $data['is_new']              = I('post.is_new')?1:0;
        $data['is_hot']              = I('post.is_hot')?1:0;
        $data['is_rec']              = I('post.is_rec')?1:0;
        //$data['groupbuy_start_date'] = strtotime($data['groupbuy_start_date']);
        //$data['groupbuy_end_date']   = strtotime($data['groupbuy_end_date']);
        $data['goods_sn']            = empty($_POST['goods_sn'])?uniqid():I('post.goods_sn');
        $data['luggage']             =I('post.luggage');//运费
        // $purchase                    =switch_money(I('post.purchase'));//采购价
        // $duties                      =$purchase*I('post.duties');//关税价
        // $arr=explode('-',I('post.luggage'));
        // $luggage1                    =$arr[0];//海运运费
        // $luggage2                    =$arr[1];//空运运费
        // $vat1                        =($purchase+$duties+$luggage1)*I('post.vat');//增值税1
        // $service_charge1             =($purchase+$duties+$luggage1+$vat1)*I('post.service_charge');//服务费1
        // $vat2                        =($purchase+$duties+$luggage2)*I('post.vat');//增值税2
        // $service_charge2             =($purchase+$duties+$luggage2+$vat2)*I('post.service_charge');//服务费2
        // $data['shop_price']          =sprintf("%.2f",$purchase+$duties+$luggage1+$vat1+$service_charge1).'-'.sprintf("%.2f",$purchase+$duties+$luggage2+$vat2+$service_charge2);//本店售价
    ...
    protected function _after_insert($data, $options){
        ...
        $sql = "UPDATE xgj_eu_goods_new SET goods_sn='{$goods_sn}' WHERE id='{$goods_id}'";
        $this->execute($sql);
    ...
    protected function _before_update(&$data, $options){
        ...
        if(isset($_FILES['face_image'])&&$_FILES['face_image']['error']==0){//有新的图片上传
             ...
             //删除y原来的封面图片
             $img_url = M('xgj_eu_goods_new')->where($options['where'])->getField('face_image');
    ...
    protected function _after_update($data, $options){
        ...
        if($old_type_id!=$type_id){
           M('xgj_eu_goods_attr')->where(array('goods_id'=>$options['where']['id']))->delete();
        }
        ...
        $attr_value       = I('post.attr_value_list');
        $attr_price       = I('post.attr_price_list');
       // var_dump('<pre>',$ga_id ,$attr_id,$attr_value,$attr_price);exit;
        //循环插入商品属性
        $model = M('xgj_eu_goods_attr');
        $len=count($attr_id);
        for($i=0; $i<$len; ++$i){
...
// die;
    ...
    public function getData(){
        // 获取分类数据
        $cate = new \Admin\Model\Eugroup\CategoryModel('category','xgj_eu_');
        $cate_list = $cate->where(array('is_show'=>1))->order('`order`')->select();
        $cate_tree = $cate->getCatTree($cate_list);
        $data['cate_tree'] = $cate_tree;

        //获取品牌数据
        $brand = new \Admin\Model\Eugroup\BrandModel;
        $brand_list = $brand->where(array('is_show'=>1))->order('`order` asc')->select();
        $data['brand_list'] = $brand_list;

        //获取商品类型
        $type = new \Admin\Model\Eugroup\TypeModel;
    ...
    public function getAll($is_delete=0){
        ...
        if(isset($_GET['send'])){
           ...
           /*if($is_groupbuy == 1){
                $map['is_groupbuy'] = array('eq',1);
            }elseif($is_groupbuy == 2){
                $map['is_groupbuy'] = array('eq',0);
            }*/
        }
        ...
        $data['goods_list'] = $this->field('g.*,c.name AS cate_name,b.name AS brand_name')->alias('g')->join('LEFT JOIN xgj_eu_category AS c ON g.cate_id=c.id')->join('LEFT JOIN xgj_eu_brand AS b ON g.brand_id=b.id')->where($map)->order('addtime DESC')->limit($page['limit'])->select();

        //处理数据
        foreach ($data['goods_list'] as &$v) {
            $v['face_image'] = getImage($v['face_image'],54,54);
            $v['addtime']    = date('Y-m-d H:i', $v['addtime']);
            $total           = M()->query('SELECT SUM(`number`) AS total FROM xgj_eu_stock WHERE goods_id='.$v['id']);
    ...
    public function getGoods($id){
       ...
       // $data['goods']['video_image'] = getImage($data['goods']['video_image']);
        ...
        //通过商品id查询出商品属性信息
        $data['attr'] = M('xgj_eu_goods_attr')->field('ga.*,a.name,a.type_id,a.mode,a.input_type,a.value_list')->alias('ga')->join('LEFT JOIN xgj_eu_attribute AS a ON ga.attr_id=a.id')->where(array('goods_id'=>$id))->order('ga.attr_id,ga.id')->select();
        $other_attr = array();
        ...
        $other_attr     = M('xgj_eu_attribute')->field('id AS attr_id,name,type_id,mode,input_type,value_list')->where($map)->order('id desc')->select();
        $data['attr']   = array_merge($data['attr'], $other_attr);

        //通过商品id查询出商品相册信息
        $data['image'] = M('xgj_eu_image')->where(array('goods_id'=>$id))->select();
        array_walk($data['image'], function(&$v,$k){
            $v['url'] = getImage($v['url'],350,350);
        });

		//通过商品id查询出商品视频
        /*$data['video'] = M('xgj_eu_video')->where(array('goods_id'=>$id))->select();
    ...
    public function deleteGoods($id){
        ...
        if($this->where(array(
            ...
            $image_list = M('xgj_eu_image')->where(array('goods_id'=>$id))->select();
            foreach ($image_list as $vv) {
                deleteImage($vv['url'], C('IMG_THUMB'));
            }
            M('xgj_eu_image')->where(array('goods_id'=>$id))->delete();

            //删除商品关联的 所有属性
            M('xgj_eu_goods_attr')->where(array('goods_id'=>$id))->delete();

            //删除商品的所有的库存
            M('xgj_eu_stock')->where(array('goods_id'=>$id))->delete();
            return true;
        }else{
            return false;
        }
