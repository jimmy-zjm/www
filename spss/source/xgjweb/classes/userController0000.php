...
class userController{
 ...
 //            }
	...
	{
		$tpl =get_smarty();
        //setcookie("preurl",$_SERVER["HTTP_REFERER"]);
	...
	function doregister(){
		...
		if($_POST){
			if($_SESSION['msg']==$_POST['msg']){
				$userdata= new userModel();
				$register=$userdata->doRegister($_POST["mobile_phone"],$_POST["userName"],$_POST["passWord"]);
			
				//如果注册成功  状态需要改变  (status)
					
				if($register==true)
				{
					$userdata->updatestatus($_COOKIE["userId"]);
				
				}
			}else{
				echo "<script>alert('验证码错误，请重新填写！')</script>";
			}
